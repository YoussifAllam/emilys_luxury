...
class Investment_Details_ViewSet(APIView):
    ...
    def post(self, request):
            ...
            if True:
                try:
                    ...
                    return Response(
                        {
                            "status": "failed",
                            "error": "Failed to register beneficiary account",
                        },
        ...
        return Response(
            {"status": "failed", "data": serializer.errors}, status=HTTP_400_BAD_REQUEST
        )
    ...
    def get(self, request):
        try:
            ...
            return Response(
                {"status": "failed", "error": "user not found"}, HTTP_404_NOT_FOUND
            )
...
class Investment_in_Dress_ViewSet(APIView):
    ...
    def post(self, request):
        is_valid = invest_in_dress_tasks.check_if_use_have_investmenter_details(
            request.user
        )
        if not is_valid:
            return Response(
                {
                    "status": "failed",
                    "error": "you should add your investment details first",
                },
                status=HTTP_400_BAD_REQUEST,
            )
        ...
        if dress_serializer.is_valid():
            ...
            investment_data = {"user": request.user.id, "dress": dress.id}
            ...
            if investment_serializer.is_valid():
                ...
                return Response(
                    {"status": "success", "dress_uuid": dress_serializer.data["id"]},
                    status=HTTP_201_CREATED,
                )
            else:
                ...
                dress.delete()
                return Response(
                    {"status": "failed", "errors": investment_serializer.errors},
                    status=HTTP_400_BAD_REQUEST,
                )
        return Response(
            {"status": "failed", "errors": dress_serializer.errors},
            status=HTTP_400_BAD_REQUEST,
        )
...
def get_payout_account(request):
    ...
    response = requests.get(
        f"https://api.moyasar.com/v1/payout_accounts", headers=headers
    )
