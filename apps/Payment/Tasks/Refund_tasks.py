...
def update_order_status_to_cancel_after_refund(target_order: order_models) -> tuple[dict, int]:
    target_order.status = 'cancelled'
    target_order.save()

def refund_moyasar_order(payment_id: uuid4, amount: int, order_id: uuid4 , target_order: order_models) -> tuple[dict, int]:
