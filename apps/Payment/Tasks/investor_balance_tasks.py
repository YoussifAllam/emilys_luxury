...
def add_investor_balance(
    order_item: order_models.OrderItem,
):  # , request_user: User_model
    SiteOwner_receivable_precentage = (
        SiteOwner_receivable_models.SiteOwner_receivable.objects.first().Percentage
    )
    ...
    investor_details_object = selectors.get_investor_detail_object(
        order_item.Target_dress
    )
    investor_Balance_object = selectors.get_investor_Balance_object(
        investor_details_object.user
    )

    total_balance, curr_balance = (
        investor_Balance_object.total_balance,
        investor_Balance_object.curr_balance,
    )
    ...
    return (
        {
            ...
            "total_balance": investor_Balance_object.total_balance,
        },
        HTTP_200_OK,
    )
