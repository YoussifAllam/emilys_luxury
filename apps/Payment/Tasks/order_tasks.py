...
def daterange(start_date, end_date):
    for n in range(
        int((end_date - start_date).days) + 2
    ):  # is +2 becuase we need 2 days after booking end date for washing and dry the dress
    ...
    try:
        for item in order.items_set.all():
            ...
            for single_date in daterange(
                item.booking_start_date, item.booking_end_date
            ):
                Dress_model.dress_busy_days.objects.create(
                    dress=dress,
                    busy_day=single_date,
                    is_temporary=True,
...
def confirm_or_cancel_temporary_bookings(order, is_success):
    busy_days = Dress_model.dress_busy_days.objects.filter(
        dress__in=[item.Target_dress for item in order.items_set.all()],
        is_temporary=True,
        order_uuid=order.uuid,
    )
    # print(busy_days , '+++++++++++')
...
def check_if_order_is_able_to_refund(
    target_order: order_models.Order,
) -> tuple[bool, str]:
    if target_order.status == "Cancelled" or target_order.status == "Delivered":
        return (
            {
                "status": "failed",
                "error": "The order is already cancelled or delivered",
            },
            HTTP_400_BAD_REQUEST,
        )
...
def hours_until_arrival(target_order: order_models.Order) -> float:
    ...
    if arrival_date:
        ...
        arrival_datetime = timezone.make_aware(
            datetime.combine(arrival_date, datetime.min.time())
        )
...
def delete_item_booking_days(item: order_models.OrderItem):
    ...
    for single_date in daterange(start_date, end_date):
        try:
            dress_busy_day = Dress_model.dress_busy_days.objects.get(
                dress=item.Target_dress, busy_day=single_date
            )
...
def Refund_order(request: HttpRequest) -> tuple[Dict, int]:
    ...
    if not is_vaild:
        return ({"status": "failed", "error": vaild_error}, HTTP_400_BAD_REQUEST)
