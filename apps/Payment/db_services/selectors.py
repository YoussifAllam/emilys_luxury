...
def get_investor_detail_object(
    dress: Dresses_models,
) -> investmenter_models.investmenter_details:
    try:
        investmenter_dress = investmenter_models.investmenter_dresses.objects.get(
            dress=dress
        )
        investmenter_detail = investmenter_models.investmenter_details.objects.get(
            user=investmenter_dress.user
        )
...
def get_investor_Balance_object(
    Target_user: User_model,
) -> investmenter_models.investmenter_balance:
    investor_Balance_object, created = (
        investmenter_models.investmenter_balance.objects.get_or_create(user=Target_user)
    )
...
def get_payment_id(target_order: order_model) -> uuid4:
    try:
        payment = payment_models.Payment.objects.get(
            order_uuid=target_order.uuid, status="paid"
        )
        return payment.id
