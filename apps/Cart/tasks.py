...
def check_if_the_startdate_does_not_passed(booking_start_date: str) -> bool:
    try:
        ...
        booking_start_date = timezone.make_aware(
            booking_start_date, timezone.get_current_timezone()
        )
    except ValueError:
...
def is_booking_duration_correct(
    booking_start_date_str: str, booking_end_date_str: str, booking_for_n_days: str
) -> bool:
...
def booking_days_is_available(
    dress: Dresses, booking_start_date_str: str, booking_end_date_str: str
) -> bool:
...
def process_is_valid(
    ...
    if booking_for_n_days not in ["3", "6", "8"]:
        return (
            {"status": "fialed", "error": "booking_for_n_days must be 3,6 or 8"},
    ...
    if not check_if_the_startdate_does_not_passed(booking_start_date):
        return (
            {"status": "fialed", "error": "The start date is passed"},
            HTTP_400_BAD_REQUEST,
        )

    if not is_booking_duration_correct(
        booking_start_date, booking_end_date, booking_for_n_days
    ):
        return (
            {"status": "fialed", "error": "The duration is not correct"},
            HTTP_400_BAD_REQUEST,
        )

    if not booking_days_is_available(dress, booking_start_date, booking_end_date):
        return (
            {"status": "fialed", "error": "The booking days are not available"},
            HTTP_400_BAD_REQUEST,
        )
