...
from ..serializers_folder.serializers import (
    HomeDressesSerializer,
    AverageRatingSerializer,
    HomeWithFavDressesSerializer,
)
...
def filter_service(
    ...
    if num_of_Stars:
        ...
        for star in num_of_Stars:
            star_conditions |= Q(
                rounded_average__gte=star, rounded_average__lt=star + 1
            )

        Target_products = (
            Target_products.annotate(
                average_rating=Avg(
                    "review_set__Rating_stars"
                )  # Corrected from dress_reviews to review_set
            )
            .annotate(rounded_average=Round("average_rating"))
    ...
    if Color:
        ...
        Target_products = Target_products.filter(Color__in=Colors).distinct()
...
def get_rating_details():
    ...
    product_averages = dress_reviews.objects.values("dress").annotate(
        avg_rating=Avg("Rating_stars")
    )
    ...
    for entry in product_averages:
        avg_rating_rounded = int(
            entry["avg_rating"]
        )  # Round to one decimal place for grouping
    ...
    for avg_rating, count in sorted(average_rating_count.items()):
        ratings_detail.append({"stars": avg_rating, "product_count": count})

    result = {"ratings_detail": ratings_detail}
...
def get_unique_data():
    ...
    unique_colors = (
        Dresses.objects.filter(is_approved=True, status="available")
    ...
    unique_designers = (
        Dresses.objects.filter(is_approved=True, status="available")
        .values_list("designer_name", flat=True)
        .distinct()
    )
...
def get_slide_data(request):
    ...
    price_data = Dresses.objects.aggregate(
        min_price=Min("price_for_3days"), max_price=Max("price_for_3days")
    )
