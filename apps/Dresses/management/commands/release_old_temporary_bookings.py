...
class Command(BaseCommand):
    ...
    def handle(self, *args, **kwargs):
        ...
        old_temporary_bookings = dress_busy_days.objects.filter(
            is_temporary=True, created_at__lt=expiration_time
        )
        ...
        not_paid_orders = Order.objects.filter(
            created_at__lt=orders_expiration_time, is_payment_completed=False
        )
        ...
        self.stdout.write(
            self.style.SUCCESS(
                f"Released {count} old temporary bookings and {orders_count} not paid orders"
            )
        )
