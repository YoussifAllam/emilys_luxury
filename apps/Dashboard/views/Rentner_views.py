...
class Get_user_num_of_points_and_code(APIView):
    ...
    def get(self, request):
        user_Data, created = user_invitation_points.objects.get_or_create(
            user=request.user
        )
...
class GetOrderShippingAddress(APIView):
    ...
    def get(self, request):
        ...
        if status_code != HTTP_200_OK:
            return Response(
                {"status": "failed", "data": "no orders address"}, status=status_code
            )

        serializer = OutputSerializers.ShippingAddressSerializer(
